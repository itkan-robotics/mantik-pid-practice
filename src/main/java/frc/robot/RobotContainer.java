...
public class RobotContainer {
  ...
  private void configureBindings() {
    ...
    m_driverController.a().whileTrue(m_armSubsystem.setAngle(Degrees.of(-5)));
    m_driverController.b().whileTrue(m_armSubsystem.setAngle(Degrees.of(15)));
    // Schedule `set` when the Xbox controller's B button is pressed,
    // cancelling on release.
    m_driverController.x().whileTrue(m_armSubsystem.set(0.3));
    m_driverController.y().whileTrue(m_armSubsystem.set(-0.3));

    /***********Elevator Subsystem***********/
    // Schedule `setHeight` when the Xbox controller's B button is pressed,
    // cancelling on release.
    m_driverController.a().whileTrue(m_elevatorSubsystem.setHeight(Meters.of(0.5)));
    m_driverController.b().whileTrue(m_elevatorSubsystem.setHeight(Meters.of(1)));
    // Schedule `set` when the Xbox controller's B button is pressed,
    // cancelling on release.
